...
class TrackOrderScreen extends StatefulWidget {
  final int? orderId; // pass a specific order, or null to show latest
...
class _TrackOrderScreenState extends State<TrackOrderScreen> {
  ...
  // ── 4 stages (matches what staff update on dashboard) ──────────
  static const List<_Stage> _stages = [
    _Stage(
      key: 'picked_up',
      label: 'Picked Up',
      subtitle: 'Your laundry has been collected',
    ...
    _Stage(
      ...
      subtitle: 'On the way back to you',
      icon: Icons.delivery_dining_rounded,
    ),
    _Stage(
      key: 'delivered',
      label: 'Delivered',
      subtitle: 'Enjoy your fresh laundry!',
  ...
  Future<void> _fetchOrder() async {
    ...
    try {
      ...
      if (widget.orderId != null) {
        url = Uri.parse('http://127.0.0.1:8000/api/orders/${widget.orderId}');
      } else {
        // Fall back to orders list, pick the latest active one
        url = Uri.parse('http://127.0.0.1:8000/api/orders');
      }
      ...
      if (response.statusCode == 200) {
        if (widget.orderId != null) {
          ...
          // Pick first active order from the list
  ...
  // Backend statuses: pending, picked_up, washing, out_for_delivery, delivered
  int _currentStageIndex(String? status) {
    switch (status) {
      ...
      default:                 return -1; // pending / not started
    }
  }
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: const Text('Try Again',
                  style: TextStyle(color: Color(0xFF1A0A2E), fontWeight: FontWeight.w800)),
            ),
          ),
        ]),
      ),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const Text('Schedule a pickup to get started',
  ...
  Widget _buildHeader(Map order, int stageIndex, bool isDelivered) {
    final statusLabel = stageIndex >= 0
                        ...
                        if (!isLast)
                          Expanded(
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: isDone ? purple : outline.withOpacity(0.4),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            style: TextStyle(
                              fontSize: 12,
                              color: isActive ? purple.withOpacity(0.7) : textGrey,
                              fontWeight: isActive ? FontWeight.w500 : FontWeight.w400,
                            ),
                          ),
                          // Show timestamp if active/done (use order timestamps if available)
                          if ((isDone || isActive) && _getStageTime(order, i) != null) ...[
                            const SizedBox(height: 4),
                            Text(
                              _getStageTime(order, i)!,
                              style: TextStyle(
                                fontSize: 11,
                                color: isDone ? textGrey : gold,
    ...
    if (isActive) {
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: purple.withOpacity(0.25), blurRadius: 10, spreadRadius: 2)],
        ),
        child: Icon(stage.icon, color: purple, size: 18),
      );
    }
    // Pending
    return Container(
      ...
      child: Icon(stage.icon, color: outline, size: 18),
    );
  }

  // Pull stage timestamp from order fields if available
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Branch
          if (branch != null)
            _detailRow(Icons.store_mall_directory_rounded, 'Branch', branch['name'] ?? '—'),

          // Pickup address
          if ((order['pickup_address'] ?? '').toString().isNotEmpty)
            _detailRow(Icons.location_on_rounded, 'Address', order['pickup_address']),

          // Pickup date
          if (order['pickup_date'] != null)
            _detailRow(Icons.calendar_today_rounded, 'Pickup Date',
                '${order['pickup_date']}${order['pickup_time'] != null ? '  ${order['pickup_time']}' : ''}'),

          // Basket size
          if (basketSize != null)
            _detailRow(Icons.shopping_basket_rounded, 'Basket Size', _capitalise(basketSize)),
            ...
            ...items.map((item) {
              ...
              return Padding(
                ...
                child: Row(children: [
                  ...
                  Expanded(
                    child: Text(_capitalise(name.toString()),
          ...
          Row(
            ...
            children: [
              ...
              Text(
                totalAmount != null
                    ? 'UGX ${_fmt(int.tryParse(totalAmount.toString()) ?? 0)}'
          ...
          if (totalAmount == null && estimatedMin != null)
            const Padding(
              padding: EdgeInsets.only(top: 4),
              child: Text('Final amount confirmed after weighing',
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 2),
            Text(value,
                style: const TextStyle(fontSize: 13, color: onSurface, fontWeight: FontWeight.w500)),
          ]),
        ),
      ]),
    );
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.support_agent_rounded, color: gold, size: 24),
          ),
          const SizedBox(width: 14),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(
                driver != null ? (driver['name'] ?? 'Your Driver') : 'Protos Support',
                style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: onSurface),
              ),
              Text(
                driver != null ? 'Your driver' : 'Branch contact',
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.call_rounded, size: 16, color: Color(0xFF1A0A2E)),
                  SizedBox(width: 6),
                  Text('Call', style: TextStyle(
                      fontSize: 13, fontWeight: FontWeight.w800, color: Color(0xFF1A0A2E))),
                ]),
              ),
            ),
        ],
