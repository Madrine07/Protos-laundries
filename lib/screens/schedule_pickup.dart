...
class _SchedulePickupScreenState extends State<SchedulePickupScreen> {
  ...
  void _schedulePickup() async {
    ...
    if (_selectedBranch == null) {
      _snack('Please select a branch', isError: true); return;
    }
    if (selectedDate == null || selectedTime == null) {
      _snack('Please select a pickup date and time', isError: true); return;
    }
    if (pickupAddress.trim().isEmpty) {
      _snack('Please enter a pickup address', isError: true); return;
    }
    if (totalItems == 0 && selectedBasket == BasketSize.none) {
      _snack('Please add items or select a basket size', isError: true); return;
    }
    if (hasClothingItems && selectedBasket == BasketSize.none) {
      _snack('Please select a basket size for your clothing items', isError: true); return;
    }
  ...
  void _resetForm() {
    setState(() {
      ...
      shirts      = 0; trousers    = 0;
      dresses     = 0; skirts      = 0;
      suits2Piece = 0; suits3Piece = 0;
      duvets      = 0; curtains    = 0;
      selectedBasket = BasketSize.none;
      _turnaround    = TurnaroundTime.standard;
    });
    _loadAddress();
    _loadBranches();
    _loadPrices();
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Overrides your profile address for this pickup only',
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: purple, foregroundColor: Colors.white, elevation: 0,
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              'For clothing items only  ·  UGX ${_fmt(_pricePerKg)}/kg',
  ...
  Widget _basketOption(BasketSize b, BuildContext sheetCtx) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? lightPurple : surfaceLow,
                ...
                children: [
                  Text(b.label,
                      style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14,
                          color: isSelected ? purple : onSurface)),
                  Text('${b.weightRange}  ·  Est. UGX ${_fmt(minP)} – ${_fmt(maxP)}',
  ...
  // ── Header (Option C: light bg + geometric bars) ──────────────
  Widget _buildHeader() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: outline.withOpacity(0.35)),
      ),
      child: Stack(
        clipBehavior: Clip.hardEdge,
        children: [
          // Decorative bar group — right side, partially clipped
          Positioned(
            right: -6,
            top: 0,
            bottom: 0,
            ...
            children: [
              ...
              Row(children: [
                ...
                const Text(
                  ...
                  style: TextStyle(
                    ...
                    letterSpacing: 1.0, color: textGrey,
                  ),
                ),
              ]),
              ...
              const Text(
                ...
                style: TextStyle(
                  ...
                  color: onSurface, height: 1.15,
                ),
              ),
              const SizedBox(height: 10),
              // Subtitle row with gold bar
              Row(children: [
                Container(
                  width: 3, height: 14,
                  decoration: BoxDecoration(
  ...
  /// CAPS label like Figma's "SELECT BRANCH"
    ...
    child: Row(children: [
      ...
      Text(text.toUpperCase(),
          style: const TextStyle(
            fontSize: 11, fontWeight: FontWeight.w700,
            letterSpacing: 0.8, color: textGrey,
          )),
    ]),
  );

  /// Section title like Figma's "Standard Items"
  Widget _sectionTitle(String title, {String? badge, Color? badgeColor}) {
    return Padding(
      ...
      child: Row(
        children: [
          Text(title,
              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: onSurface)),
          if (badge != null) ...[
            const SizedBox(width: 8),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 3),
              decoration: BoxDecoration(
                color: (badgeColor ?? purple).withOpacity(0.12),
                borderRadius: BorderRadius.circular(20),
              ),
              child: Text(badge,
                  style: TextStyle(
                    fontSize: 10, fontWeight: FontWeight.w700,
                    color: badgeColor ?? purple, letterSpacing: 0.5,
                  )),
            ),
          ],
        ],
  ...
  Widget _buildBranchSelector() {
    ...
    if (_branches.isEmpty) {
      return Container(
        ...
        child: const Text('No branches available. Please try again later.',
            style: TextStyle(color: Color(0xFF92400E), fontSize: 13)),
      );
    }

    // Pill toggle for ≤4 branches (Figma style)
                  ...
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.store_mall_directory_rounded,
                          size: 18, color: isSelected ? purple : textGrey),
                      const SizedBox(height: 4),
                      Text(branch['name'],
                          style: TextStyle(
                            fontSize: 13, fontWeight: FontWeight.w700,
                            color: isSelected ? purple : textGrey,
                          )),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _selectedBranch != null ? purple : outline, width: 1.5),
        ...
        child: DropdownButton<Map<String, dynamic>>(
          ...
          items: _branches.map((branch) {
            return DropdownMenuItem<Map<String, dynamic>>(
              value: branch,
              child: Row(children: [
                const Icon(Icons.store_mall_directory_rounded, color: purple, size: 18),
                const SizedBox(width: 10),
                Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                  Text(branch['name'],
                      style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: onSurface)),
                  if ((branch['location'] ?? '').isNotEmpty)
                    Text(branch['location'], style: const TextStyle(fontSize: 11, color: textGrey)),
                ]),
              ]),
            );
          }).toList(),
      ...
      children: [
        GestureDetector(
          ...
          child: Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              color: pickupAddress.isEmpty ? const Color(0xFFFFFBEB) : Colors.white,
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
                color: pickupAddress.isEmpty ? const Color(0xFFFDE68A) : outline,
            ...
            child: Row(
              children: [
                Container(
                  width: 42, height: 42,
                  decoration: BoxDecoration(
                    color: pickupAddress.isEmpty ? const Color(0xFFFEF3C7) : lightPurple,
                    borderRadius: BorderRadius.circular(12),
                  ),
                  child: Icon(Icons.location_on_rounded,
                      color: pickupAddress.isEmpty ? const Color(0xFFD97706) : purple, size: 20),
                ),
                const SizedBox(width: 14),
                Expanded(
                  child: _loadingAddress
                      ...
                      : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(
                            pickupAddress.isEmpty ? 'No address set' : pickupAddress,
                            style: TextStyle(
                              fontWeight: FontWeight.w600, fontSize: 14,
                              color: pickupAddress.isEmpty ? textGrey : onSurface,
                            ),
                          ),
                          const SizedBox(height: 2),
                          Text(
                            pickupAddress.isEmpty
                                ? 'Tap to enter pickup address'
                ...
                Icon(Icons.edit_rounded, size: 16,
                    color: pickupAddress.isEmpty ? const Color(0xFFD97706) : purple),
              ],
            ),
          ),
        ),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: const Row(children: [
                ...
                Expanded(
                  child: Text(
                    'Add your address in Account settings to pre-fill it here →',
  ...
  // ── Item row (Figma style with pill counter) ───────────────────
      ...
      child: Row(
        children: [
          Container(
            width: 42, height: 42,
            decoration: BoxDecoration(
              color: isSpecial ? lightPurple.withOpacity(0.6) : surfaceLow,
              borderRadius: BorderRadius.circular(12),
            ),
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(label,
                  style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: onSurface)),
              if (subtitle != null) ...[
                const SizedBox(height: 2),
                Text(subtitle, style: const TextStyle(fontSize: 12, color: purple, fontWeight: FontWeight.w500)),
              ],
            ]),
          ),
          // Pill counter (Figma style)
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              SizedBox(
                ...
                child: Text(value.toString(),
                    ...
                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: onSurface)),
              ),
  ...
  Widget _pillBtn(IconData icon, VoidCallback? onTap, bool active) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon, size: 15, color: active ? Colors.white : Colors.grey[400]),
      ),
    );
  }
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: hasBasket ? purple : outline, width: hasBasket ? 1.5 : 1),
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 8, offset: const Offset(0, 2))],
        ),
        child: Row(children: [
          const Text('🧺', style: TextStyle(fontSize: 26)),
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(
                ...
                style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14,
                    color: hasBasket ? purple : onSurface),
              ),
              const SizedBox(height: 2),
              Text(
                hasBasket
                    ? '${selectedBasket.weightRange}  ·  Est. UGX ${_fmt(minP)}–${_fmt(maxP)}'
                    : 'Tap to estimate weight of your clothing',
                style: const TextStyle(fontSize: 12, color: textGrey),
              ),
            ]),
          ),
          ...
          GestureDetector(
            ...
            child: Row(children: [
              Container(
                ...
                child: const Icon(Icons.today_rounded, color: purple, size: 20),
              ),
              const SizedBox(width: 14),
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      ...
                      color: selectedDate == null ? textGrey : onSurface,
                    ),
                  ),
                ]),
              ),
          ...
          GestureDetector(
            ...
            child: Row(children: [
              Container(
                width: 40, height: 40,
                decoration: BoxDecoration(color: lightPurple, borderRadius: BorderRadius.circular(12)),
                child: const Icon(Icons.alarm_rounded, color: purple, size: 20),
              ),
              const SizedBox(width: 14),
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      fontSize: 15, fontWeight: FontWeight.bold,
                      color: selectedTime == null ? textGrey : onSurface,
                    ),
                  ),
                ]),
              ),
  ...
  // ── Turnaround selector (Figma style) ─────────────────────────
  Widget _buildTurnaroundSelector() {
    return Row(children: [
      Expanded(child: _turnaroundCard(
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(24),
          border: Border.all(
            color: isSelected ? accentColor : outline.withOpacity(0.5),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(title, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold,
              color: isSelected ? accentColor : onSurface)),
          const SizedBox(height: 2),
          Text(subtitle, style: const TextStyle(fontSize: 12, color: textGrey)),
          const SizedBox(height: 6),
          Text(badge,
              style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800,
                  letterSpacing: 0.6, color: isSelected ? accentColor : textGrey)),
        ]),
      ),
    );
  }
  ...
  Widget _buildOrderSummary() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 16),
        if (hasClothingItems) ...[
          _summaryRow('Clothing ($clothingCount items)', 'By weight'),
        ],
        if (hasSpecial) ...[
          _summaryRow('Special items', 'UGX ${_fmt(specialFixedTotal)}'),
        ],
        if (totalItems == 0)
          const Text('No items added yet', style: TextStyle(fontSize: 13, color: textGrey)),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Text(
            showRange
                ? 'UGX ${_fmt(estimatedMin)} – ${_fmt(estimatedMax)}'
                : totalItems > 0 ? 'By weight' : '—',
  ...
  Widget _buildBottomBar() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white,
        border: Border(top: BorderSide(color: outline.withOpacity(0.4))),
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              Text('$totalItems item${totalItems == 1 ? '' : 's'}',
                  style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: onSurface)),
              const SizedBox(height: 1),
              Text(
                showRange
                    ? 'Est. UGX ${_fmt(estimatedMin)} – ${_fmt(estimatedMax)}'
                    : totalItems > 0 ? 'Weight confirmed at pickup' : 'Add items to begin',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
                    color: showRange ? purple : textGrey),
              ),
            ]),
          ),
          const SizedBox(width: 12),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // ── BRANCH ──
              _capsLabel('Select Branch'),
              _buildBranchSelector(),
              const SizedBox(height: 28),

              // ── ADDRESS ──
              _capsLabel('Pickup Address'),
              _buildAddressCard(),
              const SizedBox(height: 28),

              // ── DATE & TIME ──
              _capsLabel('Schedule Pickup'),
              ...
              Container(
                ...
                child: Row(children: [
                  const Icon(Icons.info_outline_rounded, size: 16, color: Color(0xFF735C00)),
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text(
                      _loadingPrices
                          ? 'Loading prices...'
                          : "Priced by weight · UGX ${_fmt(_pricePerKg)}/kg · Confirmed after pickup",
              ...
              _buildItemRow(
                icon: Icons.work_outline_rounded, label: 'Suit (2-Piece)', isSpecial: true,
                subtitle: _loadingPrices ? 'Loading...' : 'UGX ${_fmt(_suit2Price)} per suit',
                value: suits2Piece, onChanged: (v) => setState(() => suits2Piece = v),
              ),
              _buildItemRow(
                icon: Icons.business_center_rounded, label: 'Suit (3-Piece)', isSpecial: true,
                subtitle: _loadingPrices ? 'Loading...' : 'UGX ${_fmt(_suit3Price)} per suit',
                value: suits3Piece, onChanged: (v) => setState(() => suits3Piece = v),
              ),
              _buildItemRow(
                icon: Icons.bed_rounded, label: 'Duvet / Blanket', isSpecial: true,
                subtitle: _loadingPrices ? 'Loading...' : 'UGX ${_fmt(_duvetPrice)} each',
                value: duvets, onChanged: (v) => setState(() => duvets = v),
              ),
              _buildItemRow(
                icon: Icons.window_rounded, label: 'Curtains', isSpecial: true,
                subtitle: _loadingPrices ? 'Loading...' : 'UGX ${_fmt(_curtainPrice)} per piece',
                value: curtains, onChanged: (v) => setState(() => curtains = v),
              ),
              const SizedBox(height: 28),

              // ── TURNAROUND ──
              _sectionTitle('Turnaround Time'),
              _buildTurnaroundSelector(),
              const SizedBox(height: 28),
