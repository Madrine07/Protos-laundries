...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  int _daysUntilPickup() {
    try {
      ...
      return pickup.difference(DateTime(today.year, today.month, today.day)).inDays;
    } catch (_) { return 999; }
  }

  bool get _canModify {
    final status = _order['status'] as String?;
    return (status == 'pending' || status == 'confirmed') && _daysUntilPickup() > 1;
  }
  ...
  Future<void> _cancelOrder() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Keep Order', style: TextStyle(color: Colors.grey))),
          ElevatedButton(
  ...
  Future<void> _rescheduleOrder() async {
        ...
        builder: (ctx, setSheetState) => Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Reschedule Pickup', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold, color: Color(0xFF1A1A2E))),
              const SizedBox(height: 4),
              const Text('Select a new date and time', style: TextStyle(fontSize: 12, color: Color(0xFF9CA3AF))),
              const SizedBox(height: 20),
              GestureDetector(
                onTap: () async {
                  final picked = await showDatePicker(
                    ...
                    builder: (c, child) => Theme(data: Theme.of(c).copyWith(colorScheme: const ColorScheme.light(primary: purple)), child: child!),
                  );
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: newDate != null ? purple : const Color(0xFFE5E7EB)),
                  ),
                  child: Row(children: [
                    const Icon(Icons.calendar_today_rounded, color: purple, size: 18),
                    const SizedBox(width: 10),
                    Text(newDate == null ? 'Select new date' : DateFormat('dd MMM yyyy').format(newDate!),
                        style: TextStyle(fontWeight: FontWeight.w600, color: newDate != null ? purple : const Color(0xFF9CA3AF))),
                  ]),
                ),
              ),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: newTime != null ? purple : const Color(0xFFE5E7EB)),
                  ),
                  child: Row(children: [
                    const Icon(Icons.access_time_rounded, color: purple, size: 18),
                    const SizedBox(width: 10),
                    Text(newTime == null ? 'Select new time' : newTime!.format(ctx),
                        style: TextStyle(fontWeight: FontWeight.w600, color: newTime != null ? purple : const Color(0xFF9CA3AF))),
                  ]),
                ),
              ),
              const SizedBox(height: 20),
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: (newDate == null || newTime == null) ? null : () => Navigator.pop(ctx),
    ...
    try {
      ...
      await _apiService.rescheduleOrder(orderId: _order['id'] as int, pickupDate: formattedDate, pickupTime: formattedTime);
      setState(() { _order['pickup_date'] = formattedDate; _order['pickup_time'] = formattedTime; });
  ...
  Widget _buildShimmer() {
    return Shimmer.fromColors(
      ...
      child: ListView(
        ...
        children: [
          Container(width: double.infinity, height: 100, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18))),
          const SizedBox(height: 20),
  ...
  Widget build(BuildContext context) {
    ...
    final finalAmount   = _order['final_amount'] != null ? num.tryParse(_order['final_amount'].toString()) : null;
    final actualKg      = _order['actual_kg'] != null ? num.tryParse(_order['actual_kg'].toString()) : null;
          ...
          : ListView(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      colors: [_statusColor(status), _statusColor(status).withOpacity(0.75)],
                      begin: Alignment.topLeft, end: Alignment.bottomRight,
                    ),
                    borderRadius: BorderRadius.circular(18),
                    boxShadow: [BoxShadow(color: _statusColor(status).withOpacity(0.3), blurRadius: 12, offset: const Offset(0, 4))],
                  ),
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(_statusLabel(status), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 18)),
                ...
                _sectionCard('Order Details', [
                  _detailRow(Icons.tag_rounded, 'Order ID', '#${_order['id']}'),
                  _detailRow(Icons.store_rounded, 'Branch', branch),
                  _detailRow(Icons.calendar_today_rounded, 'Pickup Date', _order['pickup_date'] ?? '—'),
                  _detailRow(Icons.access_time_rounded, 'Pickup Time', _order['pickup_time'] ?? '—'),
                  _detailRow(Icons.location_on_rounded, 'Pickup Address', _order['pickup_address'] ?? '—'),
                  if ((_order['instructions'] ?? '').isNotEmpty)
                    _detailRow(Icons.notes_rounded, 'Instructions', _order['instructions']),
                ]),
                ...
                _sectionCard('Weight & Payment', [
                  _detailRow(Icons.scale_rounded, 'Actual Weight', actualKg != null ? '${actualKg}kg' : 'Not weighed yet'),
                  _detailRow(Icons.payments_rounded, 'Final Amount',
                      finalAmount != null ? 'UGX ${_fmt(finalAmount.toInt())}' : 'Pending weighing'),
                  _detailRow(Icons.receipt_rounded, 'Payment Status', paymentStatus ?? 'Not paid',
                      valueColor: paymentStatus == 'verified' ? const Color(0xFF059669)
                          : paymentStatus == 'pending' ? const Color(0xFFF59E0B) : Colors.grey),
                ]),
                ...
                if (actualKg != null && paymentStatus == null) ...[
                  GestureDetector(
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => OrderInvoiceFromId(orderId: _order['id']))),
                    child: Container(
                      height: 52,
                      decoration: BoxDecoration(
                        gradient: const LinearGradient(colors: [Color(0xFF6B21A8), Color(0xFF8B5CF6)]),
                        borderRadius: BorderRadius.circular(14),
                      ),
                      child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        Icon(Icons.payment_rounded, color: Colors.white, size: 18),
                        SizedBox(width: 8),
                        Text('View Invoice & Pay', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 15)),
                      ]),
                    ),
                  ),
                  const SizedBox(height: 14),
                      ...
                      Expanded(
                        child: GestureDetector(
                          ...
                          child: Container(
                            height: 50,
                            decoration: BoxDecoration(
                              color: lightPurple, borderRadius: BorderRadius.circular(12),
                              border: Border.all(color: const Color(0xFFD8B4FE)),
                            ),
                            child: _rescheduling
                                ? const Center(child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: purple)))
                                : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                                    Icon(Icons.edit_calendar_rounded, color: purple, size: 16),
                                    SizedBox(width: 6),
                                    Text('Reschedule', style: TextStyle(color: purple, fontWeight: FontWeight.w700, fontSize: 13)),
                                  ]),
                          ),
                        ),
                      ),
                          ...
                          child: Container(
                            ...
                            child: _cancelling
                                ...
                                : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                                    ...
                                    SizedBox(width: 6),
                                    Text('Cancel Order', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w700, fontSize: 13)),
                                  ]),
                          ),
                        ),
                      ),
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                            ...
                            SizedBox(width: 8),
                            Text('Call Branch Now', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
                          ]),
                        ),
                      ),
                    ]),
  ...
  Widget _sectionCard(String title, List<Widget> rows) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF6B7280), letterSpacing: 0.3)),
        const SizedBox(height: 12),
  ...
  Widget _detailRow(IconData icon, String label, String? value, {Color? valueColor}) {
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label, style: const TextStyle(fontSize: 10, color: Color(0xFF9CA3AF), fontWeight: FontWeight.w600, letterSpacing: 0.3)),
  ...
  Widget _sectionLabel(String text) {
    return Row(children: [
      Container(width: 4, height: 16, decoration: BoxDecoration(color: gold, borderRadius: BorderRadius.circular(2))),
      const SizedBox(width: 8),
      Text(text, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Color(0xFF1A1A2E))),
    ]);
  }
}